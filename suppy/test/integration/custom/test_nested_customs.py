...
def test_nested_customs():
    ...
    custom_atomic2 = CustomAtomic(uid_custom2, seh, 'Custom Node2', duration, cost, calibration_time, 1000000000, calibration_cost, type2)
    end_atomic = EndAtomic(uid_end, seh, 'End Node')

    start_custom_stream = ResourceStream(start_atomic, custom_atomic)
    custom_custom2_stream = ResourceStream(custom_atomic, custom_atomic2)
    custom2_end_stream = ResourceStream(custom_atomic2, end_atomic)

    start_atomic.register_output_stream(start_custom_stream)

    custom_atomic.register_input_stream(start_custom_stream)
    custom_atomic.register_output_stream(custom_custom2_stream)

    custom_atomic2.register_input_stream(custom_custom2_stream)
    custom_atomic2.register_output_stream(custom2_end_stream)

    end_atomic.register_input_stream(custom2_end_stream)

    network = AtomicNetwork()
    network.add_atomic(start_atomic)
    network.add_atomic(custom_atomic)
    network.add_atomic(end_atomic)
    network.add_atomic(custom_atomic2)
    ...
    assert custom_stats[IDLE_TIME] == duration
    ...
    custom_stats2 = custom_atomic2.get_stats()
    assert custom_stats2[NAME] == 'Custom Node2'
    assert custom_stats2[TYPE] == CUSTOM
    assert custom_stats2[TOTAL_CALIBRATION_COST] == 0
    assert custom_stats2[HAS_CALIBRATION] == True
    assert custom_stats2[ACTIVE_TIME] == duration * resource_count
    assert custom_stats2[TOTAL_ACTIVE_COST] == resource_count * cost
    assert custom_stats2[IDLE_TIME] == duration + 4 * calibration_time
    assert custom_stats2[CALIBRATION_COUNT] == 0
    assert custom_stats2[CALIBRATION_TIME] == 0
    ...
    assert seh.time == duration * resource_count + 4 * calibration_time + duration